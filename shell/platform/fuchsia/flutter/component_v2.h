...
class ComponentV2 final
 ...
 public:
  ...
  // TODO(akbiggs): Do we want to infer this from the package/component name or
  // take it explicitly? The compiler needs to know that stuff should go into
  // component_name/data. Putting it into CML is more flexible. Took it out of
  // Dart Runner because we had situations where people would rename the package
  // but not rename the data path.
  //
  // Options:
  //   1. Don't infer this. For JIT specify the data you need. For AOT have a
  //   convention for the names of snapshots. Could have sane defaults for
  //   filenames. Better error messaging when things go wrong is important.
  //   2. Infer it. Doesn't really make the code simpler so lean towards (1).
  //
  // Data might be too vague a name. Consider other names.
  //
  // Can do validation in flutter_dart_component to make sure that proper keys
